...
describe("YamlFilterEmojiDefinitionResultReporter", () => {
  const OUTPUT_DIR = resolveTmpDir("YamlFilterEmojiDefinitionResultReporter");
  ...
  test("應輸出 blocked.pass.report.yaml 與 blocked.summary.yaml", async () => {
    const reporter = new YamlFilterEmojiDefinitionResultReporter(OUTPUT_DIR);
