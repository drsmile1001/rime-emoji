...
describe("FilterStrategyGenderSymbol", () => {
  test("應排除含性別符號的 emoji", () => {
    ...
    const { output, blocked } = new FilterStrategyGenderSymbol().filter(defs);
...
test("性別符號本體（♀️ / ♂️）應通過", () => {
  ...
  const { blocked, output } = new FilterStrategyGenderSymbol().filter(defs);
