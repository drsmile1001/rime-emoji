...
describe("EmojiDefinitionRepoYaml", () => {
  const TEST_DIR = resolveTmpDir("EmojiDefinitionRepo.Yaml");
  const FILE_NAME = "emoji-test.yaml";
  const fullPath = join(TEST_DIR, FILE_NAME);

  let repo: EmojiDefinitionRepoYaml;

  beforeAll(async () => {
    await rm(TEST_DIR, { recursive: true, force: true });
    await mkdir(TEST_DIR, { recursive: true });
    repo = new EmojiDefinitionRepoYaml(TEST_DIR, FILE_NAME);
  });
